...
class TimelinessBaseRule(BaseRule):
    """Coercion to UTC datetime is common functionality between our timeliness functions."""
...
class TimelinessStaticBaseRule(TimelinessBaseRule):
    """
    Base rule to check whether datetime values fall between absolute start and end date boundaries (inclusive).

    Suitable where both boundaries are fixed or known in advance (e.g., events occurring in January 2024).
    All dates are treated as, or coerced to, UTC, with date-only strings assumed to be midnight. Invalid or
    unparsable datetime values are treated as missing.
    ...
    def at_least_one_date(self) -> Self:
...
class TimelinessStaticRule(TimelinessStaticBaseRule):
    """
...
class ValuesMatchStaticTimeBounds(TimelinessStaticBaseRule):
    """
    Rule to check whether datetime values in a column fall between absolute start and end date boundaries (inclusive).
...
class TimelinessRelativeBaseRule(TimelinessBaseRule):
    """
    Base rule to assess whether datetime values fall between relative time boundaries
    ...
    @model_validator(mode="after")
    def _check_only_one_reference_date_field(self) -> Self:
    ...
    @model_validator(mode="after")
    def at_least_one_timedelta(self) -> Self:
...
class TimelinessRelativeRule(TimelinessRelativeBaseRule):
    """
...
class ValuesMatchRelativeTimeBounds(TimelinessRelativeBaseRule):
    """
    Rule to assess whether datetime values fall between relative time boundaries
    from a reference date (which can be a static value or come from a column in the data source).
